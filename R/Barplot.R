relative_abundance_plot <-
  function(tabla,
    ...
    # Unir con metadatos
    columnas_a_unir <- c(col_x, facet_col, group_var)
    columnas_a_unir <- columnas_a_unir[!is.null(columnas_a_unir)]  # Eliminar NULLs
    
    tabla_long <- tabla_long %>%
      left_join(metadata %>% select(SAMPLEID, all_of(columnas_a_unir)), by = "SAMPLEID")
    ...
    # Crear gráfico
    p <- ggplot(promedio_tratamiento, aes(fill = Group, y = PromedioAbundancia, x = !!sym(group_var))) +
